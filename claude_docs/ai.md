...
- **Conditional / "N/A" free-text questions:** when the prompt is phrased contingently ("if applicable", "if yes", "if any", "If not, please type 'N/A'", "If you have/are/were…"), return `"N/A"` instead of `""` when the precondition doesn't apply. Greenhouse custom questions often reject empty values even when the underlying input isn't flagged as required (the extractor only sees `el.required`, which Greenhouse does not always set), and "N/A" is the form-convention answer. Added because Tekion forms with conditional textareas were submitting with `""` and failing validation.
...
- **Employment-section fields:** read the `employment:` list from the profile (most-recent first). Use `form_order` to tell EDUCATION date fields apart from EMPLOYMENT date fields — education fields cluster around School/Degree/Discipline, employment fields cluster around Company/Title/Employer. For "End year/month" on the current employer:
  - SELECT/COMBOBOX: prefer "Present"/"Current"/current-year ("2026") option, otherwise blank.
  - REQUIRED TEXT/NUMBER (no options): fill with current year ("2026") — DO NOT leave blank, the form rejects empty submissions even when a "currently work here" toggle exists elsewhere on the form. Greenhouse's PhonePe ingestion form was the trigger: even with the Current-role checkbox ticked, the End-date-year text field stayed required.
  - OPTIONAL TEXT/NUMBER: leave blank.
  "Currently employed here?" toggles → "Yes" when `employment[0].current` is true. Added because the resolver's generic `\bstart\s*(date\s*)?year\b` regex was matching employment date fields and quietly filling the candidate's graduation year — the regex was tightened to require `\beducation\b` and the AI now owns the disambiguation.
- **Education date sub-fields:** for **start/end year** use `education.start_year` and `education.graduation_year`. For **start/end month** the profile has no data — default to **"9"** (September start) and **"6"** (June graduation) on number inputs (placeholders `MM`, `min=1 max=12`). Always fill end-month/year for education, even when not flagged required — Roku's `weareroku.com` runs hidden HTML5 validity on the template's month/year inputs and silently aborts the submit if any of them are empty. Without this rule the AI used to return `"2017"` for a `MM` input and `""` for the year, which (a) failed range validation on month and (b) left year empty — visible as "submit click did nothing" with no POST to the form's action URL.
